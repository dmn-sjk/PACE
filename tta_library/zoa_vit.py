...
class ZOA_ViT(TTAMethod):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        self.fitness_lambda = self.args.lambda_bp
        # Optional boolean mask (len=1000) used for ImageNet-R evaluation/adaptation.
        # It is set externally in `main.py` as `adapt_model.imagenet_mask = imagenet_r_mask`.
    ...
    def forward(self, x, y=None):
        ...
        outputs, batch_mean, loss, domain_change = forward_and_get_loss(
            x,
            self.model,
            self.fitness_lambda,
            self.train_info,
...
def spsa_grad_estimate_bi(
    ...
    for si in range(sp_avg):
        ...
        *_, loss1, _ = forward_and_get_loss(
            inputs, model, fitness_lambda, train_info, shift_vector, imagenet_mask=imagenet_mask
        )
        ...
        if loss is not None:
            ...
            *_, loss2, _ = forward_and_get_loss(
                inputs, model, fitness_lambda, train_info, shift_vector, imagenet_mask=imagenet_mask
            )
            loss = loss2
