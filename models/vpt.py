...
class PromptViT(nn.Module):
    ...
    def layers_cls_features_with_prompts_stem_layer(self, x):
        x = self.vit.patch_embed(x)
        x = self.vit._pos_embed(x)
        # inject prompts
        x = self.prompt_injection(x)
        # !!end
        x = self.vit.norm_pre(x)
        return self._collect_layers_features(x), x[:, 1:].detach().clone()
    
    def stem_layer(self, x):
        x = self.vit.patch_embed(x)
        x = self.vit._pos_embed(x)
        # inject prompts
        x = self.prompt_injection(x)
        # !!end
        x = self.vit.norm_pre(x)
        return x[:,1:].clone().detach()

class DeepPromptViT(nn.Module):
    '''
    Vision Transformer with added prompts at the input layer
    '''
    def __init__(self,
        ...
        self.num_prompts = num_prompts
        self.prompt_dim = vit.embed_dim
        self.num_trans_layers = len(self.vit.blocks)

        if num_prompts > 0:
            self.prompts = nn.Parameter(torch.zeros(1, num_prompts * self.num_trans_layers, self.prompt_dim))
            # initialization adopted from vpt, https://arxiv.org/abs/2203.12119
            val = math.sqrt(6. / float(3 * reduce(mul, vit.patch_embed.patch_size, 1) + self.prompt_dim)) # noqa
            nn.init.uniform_(self.prompts.data, -val, val) # xavier_uniform initialization
    ...
    def prompt_injection(self, x, trans_layer_idx):
        p = self.prompts[:, self.num_prompts * trans_layer_idx: self.num_prompts * trans_layer_idx + self.num_prompts, :]
        x = torch.cat((
            x[:,:1,:],
            p.expand(x.shape[0],-1,-1),
            x[:,1:,:]
        ), dim=1)
        return x
